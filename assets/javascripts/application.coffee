#= require ../vendor/underscore/underscore.js
#= require ../vendor/backbone/backbone.js
#= require ../vendor/marionette/lib/backbone.marionette.js
#= require ../vendor/json3/lib/json3.min.js
#= require ../vendor/nprogress/nprogress.js
#= require ../vendor/toastr/toastr.js
#= require ../vendor/peerjs/peer.js

#= require_tree ./const
#= require_tree ./utils

#= require .//models/masks.js

#= require .//media/dat.gui.min.js
#= require .//media/utils.js
#= require .//media/numeric-1.2.6.min.js
#= require .//media/mosse.js
#= require .//media/jsfeat-min.js
#= require .//media/frontalface.js
#= require .//media/jsfeat_detect.js
#= require .//media/left_eye_filter.js
#= require .//media/right_eye_filter.js
#= require .//media/nose_filter.js
#= require .//models/model_pca_20_svm.js
#= require .//media/clm.js
#= require .//media/svmfilter_webgl.js
#= require .//media/svmfilter_fft.js
#= require .//media/mossefilter.js
#= require .//media/Stats.js
#= require .//media/face_deformer.js
#= require .//services/facedetector
#= require .//services/jthree
#= require .//services/voice_changer
#= require .//services/peer_manager

#= require .//init

#= require .//app
